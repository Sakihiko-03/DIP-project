...
def detect_hsv(img_path):
    ...
    cv2.imshow("other color", mask)

    # Apply mask to image
    color_img = cv2.bitwise_and(hsv_img, hsv_img, mask=mask)
    cv2.imshow("Apply mask to image", color_img)
    ...
    # color = np.array([color_hue, 255, 255])
    # return color, cv2.resize(color_img, (0, 0), None, 0.5, 0.5)
    return hsv
